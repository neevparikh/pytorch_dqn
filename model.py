...
class DQN_agent:
    ...
    def __init__(self, device, state_space, action_space, num_actions,
                 ...
                 epsilon_decay, epsilon_decay_start):
        """Defining DQN model
        """
    ...
    def loss_func(self, minibatch, writer=None, writer_step=None):
        # make tensors
        state_input = torch.Tensor(minibatch.state).to(self.device)
        next_state_input = torch.Tensor(minibatch.next_state).to(self.device)
        action_input = torch.Tensor(minibatch.action).to(self.device)
        ...
        q_pred_batch = self.online(state_input).gather(
            dim=1, index=action_input.long().unsqueeze(1)).squeeze(1)
        q_label_batch = reward_tensor + (self.gamma) * (
            1 - done_tensor) * self.target.max_over_actions(
                next_state_input.detach()).values
